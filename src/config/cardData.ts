export const cardData = [
  // {
  //   title: "Remainders that actually work",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },
  // {
  //   title: "See how your day looks like",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },
  // {
  //   title: "Team up and get things done",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },
  // {
  //   title: "One event calendar for all your needs",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },
  // {
  //   title: "Team up and get things done",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },
  // {
  //   title: "One event calendar for all your needs",
  //   description:
  //     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu phare tra nec, mattis ac neque.",
  //   imageUrl: "/cardImage-1.svg",
  // },

  {
    description:
      "Effortlessly organize your schedule with our event calendar feature.",
    features: [
      {
        title: "Easy Event Creation",
        description: "Quickly add events with titles, descriptions, and specific dates to keep your plans organized.",
      },
      {
        title: "Reminders & Notifications",
        description: "Set reminders to ensure you never miss an important deadline, meeting, or special occasion.",
      },
      {
        title: "Recurring Events",
        description: "Schedule recurring tasks such as weekly meetings, birthdays, and anniversaries effortlessly.",
      },
      {
        title: "Customizable Views",
        description: "Switch between daily, weekly, or monthly views for better visualization of your schedule.",
      },
      {
        title: "Sync Across Devices",
        description: "Access your events anytime, anywhere by syncing across multiple devices.",
      },
      {
        title: "Color Coding",
        description: "Categorize your events with different colors to prioritize and differentiate them easily.",
      },
      {
        title: "User-Friendly Interface",
        description: "Navigate the calendar seamlessly with an intuitive and responsive design.",
      },
      {
        title: "Collaboration Options",
        description: "Share events with family, friends, or team members to keep everyone informed.",
      },
    ],
  },
  {
    description:
      "Stay productive and organized with our to-do list feature.",
    features: [
      {
        title: "Quick Task Addition",
        description: "Easily add tasks with details such as due dates, priorities, and descriptions.",
      },
      { title: "Prioritization", description: "Categorize tasks by priority levels to focus on what matters most." },
      { title: "Progress Tracking", description: "Monitor task progress with checklists and completion percentages." },
      {
        title: "Reminders & Alerts",
        description: "Set reminders to keep yourself on track and ensure deadlines are met.",
      },
      { title: "Recurring Tasks", description: "Automate repetitive tasks such as daily workouts or weekly reviews." },
      { title: "Drag-and-Drop Organization", description: "Rearrange tasks effortlessly to fit your workflow." },
      {
        title: "Sync with Calendar",
        description: "Integrate your to-do list with the calendar to align your tasks with upcoming events.",
      },
      {
        title: "Share with Others",
        description: "Collaborate with teammates, family, or friends to manage joint tasks.",
      },
      {
        title: "User-Friendly Design",
        description: "Enjoy a seamless experience with an intuitive and minimalistic interface.",
      },
    ],
  },
  {
    description:
      "Express your ideas and share valuable insights with our blogging feature.",
    features: [
      {
        title: "Effortless Writing",
        description: "Create and format blog posts with an intuitive editor for rich text, images, and media.",
      },
      {
        title: "SEO Optimization",
        description: "Optimize your blog with meta tags, keywords, and SEO-friendly structures for better visibility.",
      },
      {
        title: "Customizable Templates",
        description: "Choose from a variety of templates to match your style and branding.",
      },
      {
        title: "Audience Engagement",
        description: "Enable comments and social sharing to interact with your readers.",
      },
      { title: "Scheduling Options", description: "Plan and schedule blog posts to publish at your preferred time." },
      {
        title: "Media Integration",
        description: "Enhance your posts with images, videos, and links to create engaging content.",
      },
      {
        title: "Analytics Tracking",
        description: "Gain insights into your blog's performance with visitor analytics and engagement metrics.",
      },
      {
        title: "Easy Editing & Updates",
        description: "Edit and update your blogs anytime with a user-friendly dashboard.",
      },
      { title: "Mobile-Friendly", description: "Create and publish blogs on the go with mobile responsiveness." },
    ],
  },
];
